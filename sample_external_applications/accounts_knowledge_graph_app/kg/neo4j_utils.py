...
def create_deployment_spec_for_neo4j():
    ...
    deployment = client.V1Deployment(
        ...
        spec=client.V1DeploymentSpec(
            replicas=1,
            selector=client.V1LabelSelector(match_labels={"app": f"neo4j-{engine_id}"}),
            template=client.V1PodTemplateSpec(
                ...
                spec=client.V1PodSpec(
                    ...
                    #         image="busybox",
                    #         command=["sh", "-c", "chown -R 8536:8536 /data"],
                    #         volume_mounts=[
                    ...
                    containers=[
                        client.V1Container(
                            ...
                            env=[
                                client.V1EnvVar(
                                    name="NEO4J_AUTH",
                                    value=f"{get_neo4j_credentails()['username']}/{get_neo4j_credentails()['password']}",
                                ),
                                client.V1EnvVar(
...
def deploy_neo4j_server():
    ...
    api_instance.create_namespaced_deployment(
        namespace=get_current_namespace(), body=deployment_spec
    )
    service_api_instance.create_namespaced_service(
        namespace=get_current_namespace(), body=service_spec
    )
...
def is_neo4j_server_up():
    neo4j_auth = (
        get_neo4j_credentails()["username"],
        get_neo4j_credentails()["password"],
    )
    with GraphDatabase.driver(
        get_neo4j_credentails()["uri"], auth=neo4j_auth
    ) as driver:
        try:
